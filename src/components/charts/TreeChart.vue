...
const colors = ['#d18d4a', '#ab935f', "#839975", "#5e9f8a"];
			...
			for (let i = 0; i < parent['children'].length; i++) {
				if(parent['toggled']) {
					pos = [...pos, {
						...
						"childrenCount": parent['children'][i].children.length,
...
function returnPos(index) {
	...
	if(pos.value[index].childrenCount){
		trianglePos = [
			{x: pos.value[index].x + (hei - l) / 2 + trileft, y: pos.value[index].y + (hei - l) / 2},
			{x: pos.value[index].x + (hei - l) / 2 + l + trileft, y: pos.value[index].y + (hei - l) / 2},
			{x: pos.value[index].x + (hei - l) / 2 + l / 2 + trileft, y: pos.value[index].y + (hei - l) / 2 + l * Math.sqrt(3) / 2}
		];
	}
	return{
		x: pos.value[index].x,
		...
		if(toggledNodeFound){
			if (parent['children'] && parent['children'].length > 0) {
				for (let i = 0; i < parent['children'].length; i++) {
					...
					dfs(parent['children'][i], true, parent['children'][i].index);
				}
			}
		}
			...
			if (parent['children'] && parent['children'].length > 0) {
				for (let i = 0; i < parent['children'].length; i++) {					
					dfs(parent['children'][i], false, parent['children'][i].index);
				}
			}
		}
		return parent
		...
		<svg class="svg-container">
			...
			>
				...
				<rect 
					...
					:rx="hei / 2" 
					:ry="hei / 2" 
					:fill="returnPos(num - index - 1).showed ? returnPos(num - index - 1).fill : `rgba(255, 255, 255, 0)`"
					:opacity="returnPos(num - index - 1).showed ? 1 : 0" 
					stroke="none" 
				/>
				<text
				...
				<polygon 
					:class="{ 'rectangles-triangle': true, [`initial-animation-rect-${num - index - 1}`]: true }"
					:points="returnPos(num - index - 1).triangle"
					:fill="returnPos(num - index - 1).showed ? `rgba(255, 255, 255, 0.65)` : `rgba(255, 255, 255, 0)`"
